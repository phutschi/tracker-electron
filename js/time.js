...
Tracker.time = {
   ...
   */
  stamp(d) {
    let f = Tracker.config.system.timeFormat

    if (f === '24') {
      return `${`0${d.getHours()}`.substr(-2)}:${`0${d.getMinutes()}`.substr(-2)}`
    } else if (f === '12') {
      return Tracker.time.twelveHours(d)
    }
  },
   ...
   */
  date(hex) {
    let a = Tracker.time.convert(Tracker.time.parse(hex))
   ...
   */
  displayDate(d) {
    let f = Tracker.config.system.calendar
   ...
   */
  duration(a, b) {
    return (Tracker.time.parse(b) - Tracker.time.parse(a)) / 3600
  }
}
